...
!!{
Implements a Eddington ratio distribution output analysis class.
!!}
  ...
  !![
  <outputAnalysis name="outputAnalysisEddingtonRatioDistribution">
   <description>A Eddington ratio distribution output analysis class.</description>
  </outputAnalysis>
  !!]
  type, extends(outputAnalysisVolumeFunction1D) :: outputAnalysisEddingtonRatioDistribution
     !!{
     A eddingtonRatioDistribution output analysis class.
     !!}
     private
  ...
  end type outputAnalysisEddingtonRatioDistribution

  interface outputAnalysisEddingtonRatioDistribution
     !!{
     Constructors for the \refClass{outputAnalysisEddingtonRatioDistribution} output analysis class.
     !!}
     module procedure eddingtonRatioDistributionConstructorParameters
     module procedure eddingtonRatioDistributionConstructorInternal
     module procedure eddingtonRatioDistributionConstructorFile
  end interface outputAnalysisEddingtonRatioDistribution

contains

  function eddingtonRatioDistributionConstructorParameters(parameters) result (self)
    !!{
    ...
    implicit none
    type            (outputAnalysisEddingtonRatioDistribution      )                              :: self
    ...
    allocate(eddingtonRatios(parameters%count('eddingtonRatios')))
    !![
    <inputParameter>
      <name>label</name>
    ...
    <inputParameter>
      <name>eddingtonRatios</name>
      <source>parameters</source>
      <variable>eddingtonRatios</variable>
      <description>The eddingtonRatios corresponding to bin centers.</description>
    </inputParameter>
    <inputParameter>
      <name>covarianceBinomialBinsPerDecade</name>
      <source>parameters</source>
      <defaultValue>10</defaultValue>
      <description>The number of bins per decade of halo mass to use when constructing Eddington ratio distribution covariance matrices for main branch galaxies.</description>
    </inputParameter>
    <inputParameter>
      <name>covarianceBinomialMassHaloMinimum</name>
      <source>parameters</source>
      <defaultValue>1.0d8</defaultValue>
      <description>The minimum halo mass to consider when constructing Eddington ratio distribution covariance matrices for main branch galaxies.</description>
    </inputParameter>
    <inputParameter>
      <name>covarianceBinomialMassHaloMaximum</name>
      <source>parameters</source>
      <defaultValue>1.0d16</defaultValue>
      <description>The maximum halo mass to consider when constructing Eddington ratio distribution covariance matrices for main branch galaxies.</description>
    </inputParameter>
    !!]
  ...
  function eddingtonRatioDistributionConstructorInternal(label,comment,eddingtonRatios,galacticFilter_,surveyGeometry_,cosmologyFunctions_,cosmologyFunctionsData,outputAnalysisPropertyOperator_,outputAnalysisDistributionOperator_,outputTimes_,blackHoleAccretionRate_,accretionDisks_,covarianceBinomialBinsPerDecade,covarianceBinomialMassHaloMinimum,covarianceBinomialMassHaloMaximum,targetLabel,functionValueTarget,functionCovarianceTarget) result(self)
    ...
    use :: Node_Property_Extractors                , only : nodePropertyExtractorEddingtonRatio
    ...
    use :: Output_Analysis_Weight_Operators        , only : outputAnalysisWeightOperatorVolumeMaximum
    implicit none
    type            (outputAnalysisEddingtonRatioDistribution              )                                          :: self
    type            (varying_string                                 ), intent(in   )                           :: label                                                 , comment
    double precision                                                 , intent(in   )          , dimension(:  ) :: eddingtonRatios
    class           (galacticFilterClass                            ), intent(in   ), target                   :: galacticFilter_
    class           (surveyGeometryClass                            ), intent(in   ), target                   :: surveyGeometry_
    class           (cosmologyFunctionsClass                        ), intent(in   ), target                   :: cosmologyFunctions_                                   , cosmologyFunctionsData
    class           (outputAnalysisPropertyOperatorClass            ), intent(inout), target                   :: outputAnalysisPropertyOperator_
    ...
    type            (nodePropertyExtractorEddingtonRatio            )               , pointer                  :: nodePropertyExtractor_
    ...
    type            (outputAnalysisWeightOperatorVolumeMaximum      )               , pointer                  :: outputAnalysisWeightOperator_
    ...
    self%binCount=size(eddingtonRatios,kind=c_size_t)
    allocate(outputWeight(self%binCount,outputTimes_%count()))
    do iBin=1,self%binCount
       outputWeight(iBin,:)=Output_Analysis_Output_Weight_Survey_Volume(self%surveyGeometry_,self%cosmologyFunctions_,outputTimes_,eddingtonRatios(iBin))
    end do
    ! Create a stellar mass property extractor.
    allocate(nodePropertyExtractor_)
    !![
    <referenceConstruct object="nodePropertyExtractor_"                           constructor="nodePropertyExtractorEddingtonRatio            ( blackHoleAccretionRate_,accretionDisks_               )"/>
    !!]
    ...
    !![
    <referenceConstruct object="outputAnalysisWeightOperator_" constructor="outputAnalysisWeightOperatorVolumeMaximum(cosmologyFunctions_,surveyGeometry_,outputTimes_,nodePropertyExtractor_)"/>
    !!]
    ...
    bufferCount=max(int(bufferWidthLogarithmic/log10(eddingtonRatios(2)/eddingtonRatios(1)))+1,bufferCountMinimum)
    ! Construct the object. We convert eddingtonRatios to log10(eddingtonRatios) here.
    self%outputAnalysisVolumeFunction1D=                                                              &
         & outputAnalysisVolumeFunction1D(                                                            &
         &                                'eddingtonRatioDistribution'//label                              , &
         &                                comment                                                   , &
         ...
         &                                .false.                                                   , &
         &                                var_str('$\log_{10}(\mathrm{\lambda})$'          ), &
         &                                var_str('$\mathrm{d}n/\mathrm{d}\log_\mathrm{\lambda}$'), &
         &                                .true.                                                    , &
         &                                .true.                                                    , &
         &                                targetLabel                                               , &
         &                                functionValueTarget                                       , &
    ...
    return
  end function eddingtonRatioDistributionConstructorInternal

  subroutine eddingtonRatioDistributionDestructor(self)
    !!{
    Destructor for the \refClass{outputAnalysisEddingtonRatioDistribution} output analysis class.
    !!}
    type(outputAnalysisEddingtonRatioDistribution), intent(inout) :: self
    ...
    !!]
    return
  end subroutine eddingtonRatioDistributionDestructor
