...
!!}
  ...
  !![
  <outputAnalysis name="outputAnalysisLuminosityFunctionNII">
   <description>A luminosity function output analysis class.</description>
  </outputAnalysis>
  !!]
  type, extends(outputAnalysisVolumeFunction1D) :: outputAnalysisLuminosityFunctionNII
     !!{
   ...
   contains
     final :: luminosityFunctionNIIDestructor
  end type outputAnalysisLuminosityFunctionNII

  interface outputAnalysisLuminosityFunctionNII
     !!{
     Constructors for the ``luminosityFunctionNII'' output analysis class.
     !!}
     module procedure luminosityFunctionNIIConstructorParameters
     module procedure luminosityFunctionNIIConstructorInternal
     module procedure luminosityFunctionNIIConstructorFile
  end interface outputAnalysisLuminosityFunctionNII

contains

  function luminosityFunctionNIIConstructorParameters(parameters) result (self)
    !!{
    Constructor for the ``luminosityFunctionNII'' output analysis class which takes a parameter set as input.
    !!}
    ...
    implicit none
    type            (outputAnalysisLuminosityFunctionNII )                              :: self
    ...
    <conditionalCall>
     <call>self=outputAnalysisLuminosityFunctionNII(label,comment,luminosities,depthOpticalISMCoefficient,galacticFilter_,surveyGeometry_,stellarSpectraDustAttenuation_,cosmologyFunctions_,cosmologyFunctionsData,outputAnalysisPropertyOperator_,outputAnalysisDistributionOperator_, starFormationHistory_,outputTimes_,hiiRegionLuminosityFunction_,hiiRegionDensityDistribution_,hiiRegionEscapeFraction_,covarianceBinomialBinsPerDecade,covarianceBinomialMassHaloMinimum,covarianceBinomialMassHaloMaximum{conditions})</call>
    ...
    !!]
    return
  end function luminosityFunctionNIIConstructorParameters

  function luminosityFunctionNIIConstructorFile(label,comment, fileName,depthOpticalISMCoefficient,galacticFilter_,surveyGeometry_,stellarSpectraDustAttenuation_,cosmologyFunctions_,cosmologyFunctionsData,outputAnalysisPropertyOperator_,outputAnalysisDistributionOperator_, starFormationHistory_,outputTimes_,hiiRegionLuminosityFunction_,hiiRegiondensitydistribution_,hiiRegionEscapeFraction_,covarianceBinomialBinsPerDecade,covarianceBinomialMassHaloMinimum,covarianceBinomialMassHaloMaximum) result (self)
    !!{
    Constructor for the ``luminosityFunctionNII'' output analysis class which reads bin information from a standard format file.
    !!}
    ...
    !!]
    return
  end function luminosityFunctionNIIConstructorFile

  function luminosityFunctionNIIConstructorInternal(label,comment,luminosities,depthOpticalISMCoefficient,galacticFilter_,surveyGeometry_,stellarSpectraDustAttenuation_,cosmologyFunctions_,cosmologyFunctionsData,outputAnalysisPropertyOperator_,outputAnalysisDistributionOperator_, starFormationHistory_,outputTimes_,hiiRegionLuminosityFunction_,hiiRegionDensityDistribution_,hiiRegionEscapeFraction_,covarianceBinomialBinsPerDecade,covarianceBinomialMassHaloMinimum,covarianceBinomialMassHaloMaximum,targetLabel,functionValueTarget,functionCovarianceTarget) result(self)
    !!{
    Constructor for the ``luminosityFunctionNII'' output analysis class which takes a parameter set as input.
    !!}
    use :: Cosmology_Functions                     , only : cosmologyFunctionsClass
    ...
    use :: Node_Property_Extractors                , only : nodePropertyExtractorLuminosityEmissionLine, nodePropertyExtractorScalarizerLuminosity
    ...
    type            (nodePropertyExtractorLuminosityEmissionLine    ), pointer                                 :: nodePropertyExtractorLuminosityEmissionLine_
    type            (nodePropertyExtractorScalarizerLuminosity      )               , pointer                  :: nodePropertyExtractor_
    ...
    lineNames(1)=var_str('nitrogenII6550' )
    lineNames(2)=var_str('nitrogenII6585' )
    cloudyTableFileName='%DATASTATICPATH%/hiiRegions/emissionLineLuminosities_BC2003_highResolution_imfChabrier.hdf5'
    component=componentTypeAll
    toleranceRelative=1.0d-3
    !![
    <referenceConstruct object="nodePropertyExtractorLuminosityEmissionLine_"                           constructor="nodePropertyExtractorLuminosityEmissionLine    (cloudyTableFileName,component,lineNames,toleranceRelative,starFormationHistory_,outputTimes_,hiiRegionLuminosityFunction_,hiiRegionDensityDistribution_,hiiRegionEscapeFraction_)"/>
    !!]
    allocate(nodePropertyExtractor_                                )
    !![
    <referenceConstruct object="nodePropertyExtractor_"                           constructor="nodePropertyExtractorScalarizerLuminosity              (1,nodePropertyExtractorLuminosityEmissionLine_                                                                               )"/>
    !!]
    ...
    self%outputAnalysisVolumeFunction1D=                                                                                               &
         ...
         &                                'luminosityFunctionNII'//label                                                          , &
         &                                comment                                                                                    , &
         &                                var_str('luminosity'                                                                      ), &
         &                                var_str('NII luminosity at the bin center'                                                 ), &
         &                                var_str('ergs/s'                                                                          ), &
         &                                ergs                                                                                       , &
         ...
         &                                .false.                                                                                    , &
         &                                var_str('$L_{NII}$ [ergs/s]'                                                 ), &
         &                                var_str('$\mathrm{d}n/\mathrm{d}\log_\mathrm{e} L_{NII}$ [$_\chi$Mpc$^{-3}$]'), &
         &                                .true.                                                                                     , &
         &                                .true.                                                                                     , &
         &                                targetLabel                                                                                , &
         &                                functionValueTarget                                                                        , &
    ...
    return
  end function luminosityFunctionNIIConstructorInternal

  subroutine luminosityFunctionNIIDestructor(self)
    !!{
    Destructor for  the ``luminosityFunctionNII'' output analysis class.
    !!}
    type(outputAnalysisLuminosityFunctionNII), intent(inout) :: self
    ...
    !!]
    return
  end subroutine luminosityFunctionNIIDestructor
