...
contains
  ...
  function emissionLineLuminosityConstructorParameters(parameters) result(self)
    ...
    <inputParameter>
      ...
      <defaultValue>var_str('/central/groups/carnegie_poc/sweeraso/Galacticus_analysesHII/datasets/static/hiiRegions/emissionLineLuminosities_BC2003_highResolution_imfChabrier.hdf5')</defaultValue>
    ...
    !!]
    self=nodePropertyExtractorLuminosityEmissionLine(cloudyTableFileName,lineNames, enumerationComponentTypeEncode(char(component),includesPrefix=.false.),toleranceRelative,starFormationHistory_,outputTimes_,hiiRegionLuminosityFunction_)
    !![
  ...
  function emissionLineLuminosityConstructorInternal(cloudyTableFileName,lineNames, component, toleranceRelative,starFormationHistory_,outputTimes_,hiiRegionLuminosityFunction_) result(self)
    !!{
    ...
    integer                                                                                          :: i
    !![
    <constructorAssign variables="cloudyTableFileName, lineNames, component, toleranceRelative, *starFormationHistory_, *outputTimes_,*hiiRegionLuminosityFunction_"/>
    !!]
    if     (                                                                                                    &
    ...
    !$ call hdf5Access%set()
    write (0,*) "FILENAME: ",char(File_Name_Expand(char(self%cloudyTableFileName)))
    ...
    self%luminositiesReduced           =+self%luminositiesReduced                                                                                                                &
         &                              /self%hiiRegionLuminosityFunction_%cumulativeDistributionFunction(rateHydrogenIonizingPhotonsMinimum,rateHydrogenIonizingPhotonsMaximum)
    do i=1,size(self%ages)
       if(self%ages(i).ge.0.01d0) then
          self%luminositiesReduced(i,:,:)=0.0d0
       end if
    end do
  ...
  function emissionLineLuminosityExtract(self,node,time,instance) result(luminosity)
    ...
    double precision                                                                                       :: sfh
    integer                                                                                                :: indexTemplate       , iLines, i
    ...
    do i=1,2
       ...
       do iLines=1,size(luminosity,dim=1)
          luminosity(iLines)=luminosity(iLines)+sum(luminosityTemplate_(iLines,:,:)*starFormationHistory%data(:,:))
          !sfh=sfh+sum(starFormationHistory%data(390,:))
          !if( (luminosity(iLines).eq.0) .and. (sum(starFormationHistory%data(390,:)) .gt. 0.0d0) ) then
       end do
    end do
    return
